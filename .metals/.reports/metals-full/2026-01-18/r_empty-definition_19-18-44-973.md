error id: file://<WORKSPACE>/src/main/java/org/example/WorkflowMetrics.java:_empty_/Host#getCpuUtilizationStats#
file://<WORKSPACE>/src/main/java/org/example/WorkflowMetrics.java
empty definition using pc, found symbol in pc: _empty_/Host#getCpuUtilizationStats#
...
public class WorkflowMetrics {
    ...
    private double calculateHostEnergy(Host host) {
        ...
        var cpuStats = host.getCpuUtilizationS@@tats();
    ...
    // Calculate the cloudlet's CPU utilization
    // CloudletLength / (VM MIPS * execTime) gives the utilization
    double cloudletLength = cloudlet.getLength();
    double vmMips = vm.getMips();
    double cloudletPes = cloudlet.getPesNumber();
    
    // CPU utilization of this cloudlet on the VM (0.0 to 1.0)
    double cloudletUtilization = Math.min(1.0, (cloudletLength / cloudletPes) / (vmMips * execTime));
    
    // Calculate VM's share of host resources
    double vmPesTotalHostPes = (double) vm.getPesNumber() / host.getWorkingPesNumber();
    
    // Cloudlet's utilization relative to the entire host
    double cloudletHostUtilization = cloudletUtilization * vmPesTotalHostPes;
    
    // Get host static power
    double hostStaticPower = 0.0;
    double hostMaxPower = powerModel.getMaxPower();
    
    if (powerModel instanceof PowerModelHostSimple) {
        hostStaticPower = ((PowerModelHostSimple) powerModel).getStaticPower();
    } else {
        // Estimate static power as ~20% of max power
        hostStaticPower = hostMaxPower * 0.2;
    }
    
    // Calculate power consumption during cloudlet execution
    // Power at cloudlet utilization level
    double dynamicPower = powerModel.getPower(cloudletHostUtilization) - hostStaticPower;
    
    // This cloudlet's share of static power (proportional to execution time)
    double staticPowerShare = (hostStaticPower * vmPesTotalHostPes);
    
    // Total power for this cloudlet
    double cloudletPower = dynamicPower + staticPowerShare;
    
    // Energy = Power × Time (in Watt-Seconds/Joules)
    return cloudletPower * execTime;
}

// Alternative simpler approach - calculate based on host total energy
private double calculateTaskEnergySimple(Cloudlet cloudlet, Vm vm) {
    ...
    // Use host's mean CPU utilization to calculate mean power
    double hostMeanUtilization = host.getCpuUtilizationStats().getMean();
    
    // If host stats are zero, calculate utilization from VMs
    if (hostMeanUtilization == 0.0) {
        double totalVmUtilization = 0.0;
        int activeVms = 0;
        
        for (Vm hostVm : host.getVmCreatedList()) {
            double vmUtil = hostVm.getCpuUtilizationStats().getMean();
            if (vmUtil > 0) {
                totalVmUtilization += vmUtil * hostVm.getPesNumber();
                activeVms++;
            }
        }
        
        if (activeVms > 0) {
            hostMeanUtilization = totalVmUtilization / host.getWorkingPesNumber();
        } else {
            // Fallback: estimate based on cloudlet
            hostMeanUtilization = (double) cloudlet.getPesNumber() / host.getWorkingPesNumber();
        }
    }
    
    // Calculate mean power during execution
    double meanPower = powerModel.getPower(hostMeanUtilization);
    
    // This cloudlet's share of host power based on its PE usage
    double cloudletPowerShare = meanPower * ((double) cloudlet.getPesNumber() / host.getWorkingPesNumber());
