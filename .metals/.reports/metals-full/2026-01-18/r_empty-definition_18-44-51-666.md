error id: file://<WORKSPACE>/src/main/java/org/example/WorkflowMetrics.java:_empty_/Datacenter#getVmCreatedList#
file://<WORKSPACE>/src/main/java/org/example/WorkflowMetrics.java
empty definition using pc, found symbol in pc: _empty_/Datacenter#getVmCreatedList#
...
private double calculateTotalEnergy() {
    ...
    for (NodeSpec node : nodes) {
        ...
        for (Host host : dc.getHostList()) {
            // Enable CPU utilization stats tracking
            host.getCpuUtilizationStats().enable();
            
            // Get total energy consumption in Watt-Seconds (Joules)
            double hostEnergy = host.getEnergyConsumption();
...
private void printDetailedMetrics() {
    ...
    private NodeSpec findNodeByVm(Vm vm) {
        for (NodeSpec node : nodes) {
            Datacenter dc = node.datacenter;
            if (dc.getVm@@CreatedList().contains(vm)) {
                return node;
            }
        }
