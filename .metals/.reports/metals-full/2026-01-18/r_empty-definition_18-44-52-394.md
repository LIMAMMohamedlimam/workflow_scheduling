error id: file://<WORKSPACE>/src/main/java/org/example/WorkflowMetrics.java:_empty_/Datacenter#getVmExecutions#
file://<WORKSPACE>/src/main/java/org/example/WorkflowMetrics.java
empty definition using pc, found symbol in pc: _empty_/Datacenter#getVmExecutions#
...
private void printDetailedMetrics() {
    ...
    private NodeSpec findNodeByVm(Vm vm) {
        for (NodeSpec node : nodes) {
            Datacenter dc = node.datacenter;
            if (dc.getVmExecutions@@().contains(vm)) {
                return node;
            }
        }
